...
def generate_all_heatmaps():
    ...
    if "Wiek" in df.columns:
        df_age = df[df["Wiek"] > 2]
        correct_age = df_age[df_age["Populacja"] == df_age[pred_col]]
        wrong_age = df_age[df_age["Populacja"] != df_age[pred_col]]

        top10_age = correct_age.sort_values(by=prob_col, ascending=False).head(10)
        bottom10_age = correct_age.sort_values(by=prob_col, ascending=True).head(10)
        wrong10_age = wrong_age.sort_values(by=prob_col, ascending=False).head(10)

        generate_heatmaps("top_10_age", top10_age, model, transform, methods, cfg, model_name, device)
        generate_heatmaps("bottom_10_age", bottom10_age, model, transform, methods, cfg, model_name, device)
        generate_heatmaps("wrong_10_age", wrong10_age, model, transform, methods, cfg, model_name, device)
    else:
        print("⚠️ Kolumna 'Wiek' nie została znaleziona w pliku Excel!")
